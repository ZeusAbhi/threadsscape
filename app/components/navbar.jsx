...
import { Menu, SearchCheck, ShoppingCart, User } from "lucide-react";
...
const validatetoken = async (user, cb) => {
  // fetch
  try {
    const userObject = JSON.parse(user);
    const response = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URI}/users/validatetoken`, {
...
const Navbar = () => {
  ...
  const [count, setCount] = useState(
    typeof Storage == "undefined" ? "" : localStorage.getItem("cartCount") || 0
  );
  ...
  useEffect(() => {
    ...
    if (userData === null) {
      setIsLoggedIn(false);
       ...
       validatetoken(userData,(val)=>{setIsLoggedIn(val);})
      // setIsLoggedIn(true)
    }
  }, []);
  return (
    <>
        ...
        <div className="flex items-center justify-center gap-12 text-lg text-black">
          <div className="absolute  left-0 flex items-center  gap-10">
            <h1 className="  flex items-center justify-center text-2xl font-bold">
              ...
              <p className="xl:hidden">ThreadScape</p>
            ...
            <Link href="/home" className=" text-sm xl:hidden">
              Home
            </Link>
            <Link href="/shop" className=" text-sm xl:hidden">
              Shop
            </Link>
          </div>
          <div className="flex flex-col">
            <Popover.Root>
              <div className="relative flex items-center justify-center">
                <input
                  className="h-7 w-[30vw] rounded-md text-center  text-sm text-black shadow-md outline-none"
                  type="text"
                  name=""
                  id=""
                  value={search}
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="hidden xl:block ">
                  <Popover.Root>
                    ...
                    <Popover.Content
                      className="z-10  my-4 h-[100px] w-[200px] gap-5  rounded-md bg-white  p-5 shadow-[0_10px_38px_-10px_hsla(206,22%,7%,.35),0_10px_20px_-15px_hsla(206,22%,7%,.2)] will-change-[transform,opacity] focus:shadow-[0_10px_38px_-10px_hsla(206,22%,7%,.35),0_10px_20px_-15px_hsla(206,22%,7%,.2),0_0_0_2px_theme(colors.white)] focus:outline-none  data-[state=open]:data-[side=bottom]:animate-slideUpAndFade data-[state=open]:data-[side=left]:animate-slideRightAndFade data-[state=open]:data-[side=right]:animate-slideLeftAndFade data-[state=open]:data-[side=top]:animate-slideDownAndFade"
                      sideOffset={5}
                    >
                      <div className="flex flex-col gap-3">
                        ...
                        <button
                          className=" h-8 w-20  rounded-md bg-red-500 transition-all hover:bg-red-600 focus:outline-none"
                ...
                <button
                  className=" h-8 w-20 rounded-md bg-red-500 transition-all hover:bg-red-600 xl:hidden"
                  onClick={handleLogout}
                >
                  <p className="text-sm font-bold text-white">Logout</p>
                </button>
      ...
      <Separator.Root
        className="   data-[orientation=vertical]:w-5px h-1 bg-slate-100 data-[orientation=horizontal]:h-px data-[orientation=vertical]:h-full data-[orientation=horizontal]:w-[100vw]"
        decorative
